...
public class UpperMove extends AutoBase {

	RobotState state;

	public UpperMove(RobotState state) {
		this.state = state;
	}

	@Override
	public RoutineBase getRoutine() {
		var path = new DrivePathPlannerRoutine("UpperMove", 0.5, 1.5, Robot.alliance, false, this.state);
		Pose2d pose;
		...
		var coneHighRoutine = new ParallelRoutine(new ArmMoveFirstStageRoutine(0.2, Arm.FirstStageState.LOWER, Arm.FirstStageState.LOWER), new SequentialRoutine(new TimeOutRoutine(0.1), new ArmMoveSecondStageRoutine(-27.5, 0.6)), new PivotSetPointRoutine(-75.8, 0.3, 0.3));
		...
		return new SequentialRoutine(
				...
				new ParallelRoutine(
						new SequentialRoutine(
								new TimeOutRoutine(0.3),
								new ParallelRoutine(
										new ArmMoveToPosRoutineTwo(-150, 0.4, Arm.FirstStageState.RAISED, Arm.FirstStageState.RAISED),
										new PivotSetPointRoutine(PivotConstants.pivotStowAngle, 0.1, 0.1))),
						path) //new BalanceChargeStationRoutine()
		/*
		//new RunIntakeRoutine(),
