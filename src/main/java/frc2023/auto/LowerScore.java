...
public class LowerScore extends AutoBase {

	RobotState state;

	public LowerScore(RobotState state) {
		this.state = state;
	}

	@Override
	public RoutineBase getRoutine() {
		var path = Robot.shouldEngage ?
				new SequentialRoutine(new DrivePathPlannerRoutine("BlueLowerScoreCubeToEngage ", 2.5, 3, Robot.alliance, true, this.state), new BalanceChargeStationRoutine()) :
				new DrivePathPlannerRoutine("BlueLowerCubeBackup", 2.5, 3, Robot.alliance, false, this.state);
		...
		return new SequentialRoutine(
				new RunIntakeRoutine(),
				new ParallelRoutine(new ArmReZeroRoutine(0.1), new PivotReZeroRoutineAuto(0.1), resetOdometry),
