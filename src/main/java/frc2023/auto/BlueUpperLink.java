...
public class BlueUpperLink extends AutoBase {

	RobotState state;

	public BlueUpperLink(RobotState state) {
		this.state = state;
	}

	@Override
	public RoutineBase getRoutine() {
		var firstPath = new DrivePathPlannerRoutine("BlueUpperToAFaceTags", 2.5, 3, Robot.alliance, false, this.state);
		...
		var thirdPath = new DrivePathPlannerRoutine("BlueUpperScoreTo2FaceTags", 2.5, 3, Robot.alliance, true, this.state);
		var finalPath = new DrivePathPlannerRoutine("BlueUpper2ToScoreFaceTags", 2, 3, Robot.alliance, true, this.state);
		Pose2d pose;
		if (Robot.alliance == DriverStation.Alliance.Blue) {
			pose = new Pose2d(1.79, 4.42, Rotation2d.fromDegrees(180));
		} else {
			pose = new Pose2d(1.79, 8 - 4.42, Rotation2d.fromDegrees(180));
		}
		var resetOdometry = new ResetOdometry(state.robotPoseMeters);
		...
		var pickupCone2 = new ParallelRoutine(
				new ArmMoveToPosRoutineTwo(-139, 0.5, Arm.FirstStageState.RAISED, Arm.FirstStageState.RAISED),
				new PivotSetPointRoutine(17.5, 0.5, 0.5),
				new RunIntakeRoutine());
		...
		var resetArmThree = new ParallelRoutine(new SequentialRoutine(new TimeOutRoutine(0.2), new ArmMoveToPosRoutineTwo(-160, 0.6, Arm.FirstStageState.RAISED, Arm.FirstStageState.RAISED)), new PivotSetPointRoutine(PivotConstants.pivotStowAngle, 0.1, 0.1));

		var coneHighRoutine = new ParallelRoutine(new ArmMoveToPosRoutineTwo(-73.0, 0.6, Arm.FirstStageState.RAISED, Arm.FirstStageState.RAISED), new PivotSetPointRoutine(-41, 0.6, 0.5));
		...
		var coneHighRoutine2 = new ParallelRoutine(new ArmMoveToPosRoutineTwo(-73.0, 0.6, Arm.FirstStageState.RAISED, Arm.FirstStageState.RAISED), new PivotSetPointRoutine(-41, 0.6, 0.5));
		...
		return new SequentialRoutine(
				...
				new ParallelRoutine(
						new SequentialRoutine(new TimeOutRoutine(2.5), pickupCone),
						firstPath),
				new ParallelRoutine(secondPath, resetArmTwo, new SequentialRoutine(new TimeOutRoutine(1.8), coneHighRoutine)),
				new TimeOutRoutine(0.3), new IntakeOuttakeRoutine(0.3),
				new ParallelRoutine(thirdPath, new SequentialRoutine(new TimeOutRoutine(1.7), pickupCone2)),
				new ParallelRoutine(resetArmThree, new ParallelRoutine(finalPath), new SequentialRoutine(new TimeOutRoutine(1.7), coneHighRoutine2)),
				new TimeOutRoutine(0.3), new IntakeOuttakeRoutine(0.3),
				coneHighRetractRoutine
